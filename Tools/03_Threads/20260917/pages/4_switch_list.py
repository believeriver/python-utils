...
def render_switch_list_page(config: dict, role: str):
    ...
    with st.expander("🔎 検索・フィルタ", expanded=True):
        ...
        with f_col1:
            hostname_filter = st.text_input("ホスト名（部分一致）", placeholder="例: KSY")
        with f_col2:
        ...
        with f_col5:
            model_options = sorted(df["機種"].dropna().unique().tolist())
            sel_models = st.multiselect("機種", options=model_options)
        with f_col6:
    ...
    if sel_models:
        filtered = filtered[filtered["機種"].isin(sel_models)]
    if sel_status:
