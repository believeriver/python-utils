...
DUMMY_ARP: list[dict] = [
    {"VLAN": 10,  "インターフェース": "Vlan10",  "IPアドレス": "10.0.10.1",    "MACアドレス": "00:1a:2b:3c:4d:01"},
    {"VLAN": 10,  "インターフェース": "Vlan10",  "IPアドレス": "10.0.10.101",  "MACアドレス": "00:1a:2b:3c:4d:02"},
    {"VLAN": 10,  "インターフェース": "Vlan10",  "IPアドレス": "10.0.10.102",  "MACアドレス": "00:1a:2b:3c:4d:03"},
    {"VLAN": 20,  "インターフェース": "Vlan20",  "IPアドレス": "10.0.20.1",    "MACアドレス": "00:aa:bb:cc:dd:01"},
    {"VLAN": 20,  "インターフェース": "Vlan20",  "IPアドレス": "10.0.20.50",   "MACアドレス": "00:aa:bb:cc:dd:02"},
    {"VLAN": 20,  "インターフェース": "Vlan20",  "IPアドレス": "10.0.20.51",   "MACアドレス": "00:aa:bb:cc:dd:03"},
    {"VLAN": 30,  "インターフェース": "Vlan30",  "IPアドレス": "10.0.30.1",    "MACアドレス": "a0:b0:c0:d0:e0:01"},
    {"VLAN": 40,  "インターフェース": "Vlan40",  "IPアドレス": "192.168.40.1", "MACアドレス": "fe:dc:ba:98:76:01"},
    {"VLAN": 40,  "インターフェース": "Vlan40",  "IPアドレス": "192.168.40.10","MACアドレス": "fe:dc:ba:98:76:02"},
    {"VLAN": 100, "インターフェース": "Vlan100", "IPアドレス": "172.16.100.1", "MACアドレス": "11:22:33:44:55:01"},
    {"VLAN": 100, "インターフェース": "Vlan100", "IPアドレス": "172.16.100.2", "MACアドレス": "11:22:33:44:55:02"},
    {"VLAN": 100, "インターフェース": "Vlan100", "IPアドレス": "172.16.100.3", "MACアドレス": "11:22:33:44:55:03"},
]
...
def fetch_arp_dataframe() -> tuple[pd.DataFrame, list[str]]:
    ...
    df = pd.DataFrame(rows) if rows else pd.DataFrame(
        columns=["VLAN", "インターフェース", "IPアドレス", "MACアドレス"]
    )
    return df, errors
...
def render_arp_page(config: dict, username: str):
    ...
    # ---- 全画面高さ: DataFrameをウィンドウいっぱいに広げるCSS ----
    st.markdown("""
    <style>
    /* ヘッダ・フッタの余白を詰める */
    .block-container {
    ...
    with st.expander("🔎 フィルタ", expanded=True):
        ...
        with f2:
            sel_if = st.multiselect(
                "インターフェース",
                options=sorted(df["インターフェース"].unique()),
            )
        with f3:
    ...
    if sel_if:
        filtered = filtered[filtered["インターフェース"].isin(sel_if)]
    if ip_filter:
    ...
    # テーブル（height="全画面" は CSS で制御。ここでは大きめの値を指定）
    st.dataframe(
        filtered.sort_values(["VLAN", "IPアドレス"]),
        use_container_width=True,
        hide_index=True,
        height=800,   # CSSと組み合わせてウィンドウサイズに追従
        column_config={
